  ...
  <div class="card-body">
    ...
    <div style="display:flex; flex-wrap:wrap; gap:24px; margin-bottom:24px;">
      ...
      <div>
        ...
        <div style="font-weight:600; font-size:16px;"><?= esc($teacherName ?: '—') ?></div>
      </div>
      <div>
              ...
              <table class="data-table">
                ...
                <tbody>
                  <?php foreach ($sched['tasks'] as $t): ?>
                    <?php
                      // Friendly label for the independence level
                      $levelLabels = [
    ...
    <!-- Classroom sessions in this semester -->
    <h2 style="font-size:18px; border-bottom:1px solid #e2e8f0; padding-bottom:8px; margin-top:28px;">
      Classroom Sessions (<?= count($sessions) ?>)
    </h2>
    <?php if (empty($sessions)): ?>
      <p style="color:#64748b;">No classroom sessions recorded in this semester.</p>
    <?php else: ?>
      <table class="data-table">
        <thead>
          <tr><th>Date</th><th>Subject</th><th>Notes</th></tr>
        </thead>
        <tbody>
    ...
    <!-- Observations in this semester -->
    <h2 style="font-size:18px; border-bottom:1px solid #e2e8f0; padding-bottom:8px; margin-top:28px;">
      Academic Observations (<?= count($observations) ?>)
    </h2>
    <?php if (empty($observations)): ?>
      <p style="color:#64748b;">No observations recorded in this semester.</p>
    <?php else: ?>
      <ul style="padding-left:18px;">
        <?php foreach ($observations as $o): ?>
          <li style="margin-bottom:6px;">
            <span style="color:#64748b; font-size:12px;"><?= date('d M Y', strtotime($o->created_at)) ?>:</span>
            <?= esc($o->observation) ?>
          </li>
        <?php endforeach; ?>
      </ul>
    ...
    <div style="margin-top:40px; display:flex; justify-content:space-between; gap:40px;">
      <div style="flex:1; border-top:1px solid #94a3b8; padding-top:6px; color:#64748b; font-size:13px;">Teacher signature</div>
